...
public interface CityCodeRepository extends JpaRepository<CityCode, String> {
        ...
        Page<CityCode> findByCountryAndKorContaining(@Param("country") String country, @Param("kor") String kor,
                        Pageable pageable);
        ...
        // FROM AirportCode a
        // JOIN a.cityCode c
        // WHERE LOWER(c.cityKor) LIKE LOWER(CONCAT('%', :keyword, '%'))
        // OR LOWER(c.cityCode) LIKE LOWER(CONCAT('%', :keyword, '%'))
        // OR LOWER(a.airportKor) LIKE LOWER(CONCAT('%', :keyword, '%'))
